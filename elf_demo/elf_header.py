...
def readElfHeader(f):
  ...
  #print(ident_data)
  fmt = None
  if int(ident_data[4]) == 1:
    elf_class = 32
    fmt = fmt32
    data = f.read(struct.calcsize(fmt32))
  elif int(ident_data[4]) == 2:
    elf_class = 64
    fmt = fmt64
    data = f.read(struct.calcsize(fmt64))
  if int(ident_data[5]) == 1: #little-endian
    fmt = '<' + fmt_ident + fmt
    end_char = '<'
  elif int(ident_data[5]) == 2: #big-endian
    fmt = '>' + fmt_ident + fmt
    end_char = '>'
...
def readShHeaders(f,elf_hdr):
  ...
  for hdr in sh_hdrs:
    ...
    hdr['sh_name'] = shstr[offset:offset+shstr[offset:].index(0x0)]
    global shidx_strtab
